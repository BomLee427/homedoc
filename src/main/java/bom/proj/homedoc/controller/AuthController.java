...
public class AuthController {
    ...
    @PostMapping("/signin")
    public ResponseEntity<CommonResponseDto<AuthorizeResponseDto>> signInV1(
            @Validated @RequestBody SigninRequestDto dto
    ) {
        TokenDto tokenDto = authService.getToken(dto);
        //TODO: 헤더 세팅 필요한지 확인
        return ResponseEntity.ok(CommonResponseDto.getResponse(AuthorizeResponseDto.builder()
                            .accessToken(tokenDto.getAccessToken())
                            .refreshToken(tokenDto.getRefreshToken()).build()));
    }

    @PostMapping("/refresh")
    public ResponseEntity<CommonResponseDto<AuthorizeResponseDto>> tokenRefreshV1(
